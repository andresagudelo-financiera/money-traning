...
export default function WhatsAppButton() {
    ...
    const handleClick = () => {
        ...
        if (typeof window !== 'undefined' && (window as any).fbq) {
            (window as any).fbq('trackCustom', 'WhatsAppSupportClick', {
                source: source,
                page: window.location.pathname
            });
        }

        const message = `Hola Claudia! Vengo de ${source} y me gustaría recibir soporte 1-1 sobre Money Training.`;
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[60] group">
            ...
            <button
                onClick={handleClick}
                className="relative w-14 h-14 bg-[#25D366] text-white rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 animate-[pulse_2s_infinite]"
